...
elevation rasters. It is supposed to be used within an FME PythonCaller and
already contains reading and writing functions wrapped to the FME Python API.
It can be used to generate a DTM from an unclassified LIDAR pointcloud, assuming
that this pointcloud has already been turned into an evenly spaced minimum
elevation raster (picking minimum z for each cell) which is used as input to the
PythonCaller.
...
class FeatureProcessor(object):
    ...
    def input(self, feature):
        ...
        MyFilter = ProgressiveMorphologicalFilter(self.data, parameters)
        self.output = MyFilter.filter()
        return
